...
# Only remove cellular/baseband/modem packages. Keep Wi-Fi/WCNSS packages.
DISABLE_PKGS="
...
MODEM_FW_SYMBOLS="$(grep -E '^CONFIG_(PACKAGE|DEFAULT)_qcom-msm8916-modem-openstick-.*-firmware=y' .config | sed -E 's/^CONFIG_(PACKAGE|DEFAULT)_//' | sed -E 's/=y$//' | sort -u || true)"
...
grep -E 'modem|qmi|mbim|wwan|qrtr|bam-dmux|comgt|rmtfs|sms|DbusSms' .config || true
